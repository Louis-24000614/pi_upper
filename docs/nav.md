...
禁止把策略写成「永远逆时针前进、从不改边」。绕向只能作为 **初始偏好**；一旦边 `blocked`，必须以拓扑重搜结果为准，允许改道、短时反向（倒车段）。
...
| `BACKUP` | 短距倒车（开环距离或退向上一拓扑节点）；禁止盲目长距倒车 |
| `REPLAN_GLOBAL` | `set_edge_blocked` → Dijkstra/A\* → 更新边序列 |
...
# 调试快照（可供 GUI）
get_nav_snapshot() -> {
  mode, current_edge, blocked_edges,
  global_path, local_path_ok,
...
| 仅拓扑换边 | 障碍在边中段时过粗；需倒回节点的规则补齐后可作 **第一期** |
...
| 倒车最大距离与是否允许上坡台倒车 | `BACKUP` 安全包络 | 队内场测 |
